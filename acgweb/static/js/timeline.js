...
links.Timeline.StepDate.prototype.roundToMinor = function() {
    ...
    switch (this.scale) {
        ...
        case links.Timeline.StepDate.SCALE.DAY:          //this.current.setDate(this.current.getFirstDateOfThisWeek());// intentional fall through
    ...
    if (this.scale == links.Timeline.StepDate.SCALE.DAY) {
        var tmp = this.current.getFirstDateOfThisWeek();
        //console.log(tmp,tmp.mon,tmp.day)
        this.current.setMonth(tmp.mon);
        this.current.setDate(tmp.day);
        this.current.setHours(0);
        this.current.setMinutes(0);
        this.current.setSeconds(0);
        this.current.setMilliseconds(0);
    }
    else if (this.scale == links.Timeline.StepDate.SCALE.WEEKDAY) {
        var tmp = this.current.getFirstDateOfThisWeek();
        //console.log(tmp,tmp.mon,tmp.day)
        this.current.setMonth(tmp.mon);
        this.current.setDate(tmp.day);
    ...
    if (this.step != 1) {
        ...
        switch (this.scale) {
            ...
            case links.Timeline.StepDate.SCALE.WEEKDAY:      this.current.setDate((this.current.getDate()-1) - (this.current.getDate()-1) % this.step + 1); break;//this.current.setDate(this.current.getFirstSundayOfThisMonth()); break;
            //    var tmp = this.current.getFirstDateOfThisWeek();
            //    //console.log(tmp,tmp.mon,tmp.day)
            //    this.current.setMonth(tmp.mon);
            //    this.current.setDate(tmp.day+this.step);
            ...
            case links.Timeline.StepDate.SCALE.DAY:          //this.current.setDate((this.current.getDate()-1) - (this.current.getDate()-1) % this.step + 1); break;
                ...
                //console.log(tmp,tmp.mon,tmp.day)
                this.current.setMonth(tmp.mon);
                this.current.setDate(tmp.day+7*(1-this.current.getWeek()%2));
                //this.current.setDate((this.current.getDate()-1) - (this.current.getDate()-1) % (7*this.step) + 1);
                break;
...
links.Timeline.StepDate.prototype.next = function() {
    ...
    switch (this.scale) {
        ...
        case links.Timeline.StepDate.SCALE.WEEKDAY:      this.current.setDate(this.current.getDate() + this.step); break;//this.current.setDate(this.current.getFirstSundayOfThisMonth()+this.current.getDate() + this.step); break;// intentional fall through
        case links.Timeline.StepDate.SCALE.DAY:          this.current.setDate(this.current.getDate() + (7*this.step)); break;
    ...
    if (this.step != 1) {
        ...
        switch (this.scale) {
            ...
            case links.Timeline.StepDate.SCALE.WEEKDAY:      if(this.current.getDate() < this.step+1) {// this.current.setDate(this.current.getDate()-7+1); break;//if(this.current.getDate() < this.step+1) this.current.setDate(1); break;// intentional fall through
                var tmp = this.current.getFirstDateOfThisWeek();
                this.current.setMonth(tmp.mon);
                this.current.setDate(tmp.day);
            } break;
            case links.Timeline.StepDate.SCALE.DAY:          if(this.current.getDate() < (7*this.step)+1) {
                var tmp = this.current.getFirstDateOfThisWeek();
                this.current.setMonth(tmp.mon);
                this.current.setDate(tmp.day);
            } break;
...
links.Timeline.StepDate.prototype.setMinimumStep = function(minimumStep) {
    ...
    if (stepDay*7 > minimumStep)            {this.scale = links.Timeline.StepDate.SCALE.DAY;         this.step = 2;}
    if (stepDay*7/2 > minimumStep)            {this.scale = links.Timeline.StepDate.SCALE.DAY;         this.step = 1;}
//    if (stepDay*2 > minimumStep)            {this.scale = links.Timeline.StepDate.SCALE.WEEKDAY;         this.step = 4;}
//    if (stepDay > minimumStep)              {this.scale = links.Timeline.StepDate.SCALE.WEEKDAY;         this.step = 2;}
    if (stepDay > minimumStep)            {this.scale = links.Timeline.StepDate.SCALE.WEEKDAY;     this.step = 1;}
...
links.Timeline.StepDate.prototype.snap = function(date) {
    if (this.scale == links.Timeline.StepDate.SCALE.YEAR) {
        ...
        switch (this.step) {
            case 2:
                if(date.getWeek()%2==0)
                    date.setDate(date.getFirstSundayOfThisMonth()+7);
                else
                    date.setDate(date.getFirstSundayOfThisMonth());
                break;
            default:
                date.setDate(date.getFirstSundayOfThisMonth()); break;
        }
        date.setHours(0);
        date.setMinutes(0);
        date.setSeconds(0);
...
links.Timeline.StepDate.prototype.isMajor = function() {
    switch (this.scale) {
        ...
        case links.Timeline.StepDate.SCALE.WEEKDAY: // intentional fall through
            ...
            return (this.current.getDay() == 0);
        case links.Timeline.StepDate.SCALE.DAY:
            if(this.step == 1)
                return (this.current.getDate() == this.current.getFirstSundayOfThisMonth());
            else if(this.step == 2)
                return ((this.current.getDate() == this.current.getFirstSundayOfThisMonth()) || 
                    (this.current.getDate() == this.current.getFirstSundayOfThisMonth()+7));
            //return (this.current.getDate() == this.current.getFirstSundayOfThisMonth());
...
links.Timeline.StepDate.prototype.getLabelMinor = function(options, date) {
    ...
    switch (this.scale) {
        ...
        case links.Timeline.StepDate.SCALE.HOUR:
            return this.addZeros(date.getHours(), 2) + ":" + this.addZeros(date.getMinutes(), 2);
        case links.Timeline.StepDate.SCALE.WEEKDAY:      return (date.getMonth()+1) + "-" + date.getDate() + " " + options.DAYS[date.getDay()] ;
        case links.Timeline.StepDate.SCALE.DAY:          return  "第" +date.getWeek() + "周";
        case links.Timeline.StepDate.SCALE.MONTH:        return options.MONTHS[date.getMonth()];   // month is zero based
...
/**
 * Returns the first Sunday for this month. 
 * @return int
 */
Date.prototype.getFirstSundayOfThisMonth = function () {
    var objDate = new Date(this.getFullYear(),this.getMonth(),1);
    var firstSunday = 7-objDate.getDay();
    //console.log(objDate,this.getMonth(),firstSunday);
    if (firstSunday==7) firstSunday = 0;
    return firstSunday+1;
};

/**
 * Returns the first Sunday for this week. 
 * @return int
 */
Date.prototype.getFirstDateOfThisWeek = function () {
    var objDate = new Date(this.getFullYear(),this.getMonth(),this.getDate());
    var dateSunday = this.getDate() - this.getDay();
	objDate.setDate(dateSunday);
    if (dateSunday<1) {
        objDate.setDate(objDate.getDate()-7);
    }
    //console.log(objDate);
    return ({mon:objDate.getMonth(),day:objDate.getDate()});
};