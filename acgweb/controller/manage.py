...
def manage():
    ...
    try:
        ...
        param['last_cron'] = 0
    print config.BASE_DIR+'data/last_cron.time'
...
def salarymanage():
    ...
    sql1 = "select c.uid, b.id, b.start_time, b.venue, b.title, b.end_time-b.start_time as last_time from duty as a left join activity as b on a.aid = b.id left join member as c on c.uid = a.uid where b.end_time != '0' and b.start_time > '%d' and b.end_time < %d and a.status = '11' order by b.start_time " % (int(starttime), int(endtime))
    #print sql1
    ...
    for i in res1:
        if not salalist.has_key(i.uid):
            salalist[i.uid] = []
        salalist[i.uid].append({'aid':i.id,'start_time':i.start_time,'venue':i.venue,'title':i.title,'last_time':i.last_time})
    #print salalist
    sql2 = "select c.uid, c.name, c.credit_card, sum(b.end_time-b.start_time) as totaltime from duty as a left join activity as b on a.aid = b.id left join member as c on c.uid = a.uid where b.end_time != '0' and b.start_time > '%d' and b.end_time < %d and a.status = '11' group by a.uid order by totaltime " % (int(starttime), int(endtime))
    #print sql2
